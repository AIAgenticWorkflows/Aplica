...
export function SimpleOrb() {
  ...
  return (
    ...
    >
      {/* Hero orb — golden yellow (logo primary), centre */}
      <motion.div
      ...
      >
        <div
          ...
          style={{ background: "radial-gradient(circle, #F5A623 0%, transparent 70%)", opacity: 0.30 }}
        />
      </motion.div>

      {/* Secondary hero orb — cobalt blue (logo arc), right */}
      <motion.div
        style={{ opacity: heroOrbOpacity, top: "45%", left: "62%" }}
      ...
      >
        <div
          className="w-full h-full rounded-full"
          style={{ background: "radial-gradient(circle, #1B5FA8 0%, transparent 70%)", opacity: 0.22 }}
        />
      </motion.div>

      {/* Accent orb — crimson red (logo A mark), left */}
      <motion.div
        style={{ opacity: heroOrbOpacity, top: "48%", left: "20%" }}
      ...
      >
        <div
          className="w-full h-full rounded-full"
          style={{ background: "radial-gradient(circle, #C8392B 0%, transparent 70%)", opacity: 0.18 }}
        />
      </motion.div>

      {/* Mission section — logo gradient wash */}
      <motion.div
      ...
      >
        <motion.div
          ...
          style={{ background: "radial-gradient(circle, rgba(245,166,35,0.10) 0%, rgba(200,57,43,0.07) 50%, transparent 70%)" }}
          animate={{ rotate: [0, 360], scale: [1, 1.2, 1] }}
          transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
        />
        <motion.div
          className="absolute w-[400px] h-[400px] rounded-full blur-2xl top-12 left-12"
          style={{ background: "radial-gradient(circle, rgba(27,95,168,0.10) 0%, transparent 70%)" }}
          animate={{ rotate: [360, 0], scale: [1.2, 1, 1.2] }}
          transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
        />
      </motion.div>

      {/* Values section — four logo-colour orbs */}
      <motion.div
      ...
      >
        ...
        <motion.div
          ...
          style={{ background: "radial-gradient(circle, rgba(27,95,168,0.25) 0%, transparent 70%)" }}
          animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }}
          transition={{ duration: 4, delay: 1, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-1/4 left-1/6 w-32 h-32 rounded-full blur-2xl"
          style={{ background: "radial-gradient(circle, rgba(200,57,43,0.25) 0%, transparent 70%)" }}
          animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }}
          transition={{ duration: 4, delay: 2, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-1/4 right-1/6 w-32 h-32 rounded-full blur-2xl"
          style={{ background: "radial-gradient(circle, rgba(42,171,220,0.25) 0%, transparent 70%)" }}
          animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }}
